...
extension Snapshotting where Value == SCNScene, Format == UIImage {
    ...
    ///   - precision: The percentage of pixels that must match.
    ///   - size: The size of the scene.
    public static func image(precision: Float = 1, perceptualPrecision: Float = 0, size: CGSize, png: Bool) -> Snapshotting {
        return .scnScene(precision: precision, perceptualPrecision: perceptualPrecision, size: size, png: png)
    }
}
#endif

fileprivate extension Snapshotting where Value == SCNScene, Format == Image {
    static func scnScene(precision: Float, perceptualPrecision: Float = 0, size: CGSize, png: Bool) -> Snapshotting {
        return Snapshotting<View, Image>.image(precision: precision, perceptualPrecision: perceptualPrecision, png: png, interfaceStyle: .light).pullback { scene in

        ...
        return Snapshotting(
            ...
            asyncSnapshot: { value in
                Async { callback in
                    ...
                    (strategy.snapshot(value) as! Async<UIImage>).run( { image in
                        oldImage = image
                    })
                    ...
                    strategy.snapshot(value).run( {newValue in
                        let copyOldImage = oldImage
                        let copyNewImage = newValue as! UIImage
